...
from internal.summirizer import summarise
import io 
import uuid
...
class KafkaWorker():
    ...
    async def send(self, message: dict):
        ...
        await self.producer.send_and_wait(kafkaConfig.TOPIC_PRODUCE, value=json.dumps(message).encode("utf-8"))
    ...
    async def kafka_processor(self):
        try:
            async for msg in self.consumer:
                ...
                # Процесс суммаризации: получение из s3 аудио файла ->  транскрибация -> запись в бд -> запись в s3-> вывод информации о записи
                audio_data = await s3_client.download_file(object_name = data["s3_filename"])
                
                result_text = summarise(audio_data)

                summary_name = uuid.uuid4()
                craete_summary(db = get_db(), audio_id= data["audio_id"], filename = summary_name)
                ...
                text_file.seek(0)
                s3_client.upload_file(file_data = text_file, object_name = summary_name, file_length = len(binary_text), content_type="txt/plain")
